...
class UploadAsyncMixin:
    ...
    async def _upload_part(
        ...
        data.add_field("file", part_data, filename=file.name, content_type=file.content_type)
        ...
        try:
            async with session.post(upload_url, data=data, headers=headers, proxy=self.proxy) as response:
    ...
    async def upload_2c_personal(
        ...
        Returns:
            文件 ID (fid)
        """
        return await self.upload_2c(SPACE_TYPE_PERSONAL, file, target_dir_id, "", opt)

    async def upload_2c_family(
        self: WoClientAsync,
        file: Upload2CFile,
        target_dir_id: str,
        family_id: str,
        ...
        Returns:
            文件 ID (fid)
        """
        return await self.upload_2c(SPACE_TYPE_FAMILY, file, target_dir_id, family_id, opt)


# 将异步上传方法注入到 WoClientAsync 类
def _inject_upload_async_methods():
    """将异步上传方法注入到 WoClientAsync 类"""
    WoClientAsync._init_zone_url = lambda self: UploadAsyncMixin._init_zone_url(self)
    WoClientAsync._random_chars = UploadAsyncMixin._random_chars
    WoClientAsync.upload_2c = UploadAsyncMixin.upload_2c
    WoClientAsync._upload_part = UploadAsyncMixin._upload_part
    WoClientAsync.upload_2c_personal = UploadAsyncMixin.upload_2c_personal
    WoClientAsync.upload_2c_family = UploadAsyncMixin.upload_2c_family
